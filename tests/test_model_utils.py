...
def test_compare_models_returns_dataframe(trained_model):
    import pandas as pd
    model, X_test, y_test = trained_model
    r1 = evaluate_model("LR_A", model, X_test, y_test, "DataA")
    r2 = evaluate_model("LR_B", model, X_test, y_test, "DataB")
...
def test_log_cv_results_returns_correct_keys(trained_model):
    model, X_test, y_test = trained_model
    X, y = make_classification(
        n_samples=500, n_features=10, weights=[0.9, 0.1], random_state=42
    )
